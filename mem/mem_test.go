...
var tdBE = [...]testData{
	{func(b *sys.Bus, addr mirv.Address) error { return b.Write64(addr, 0) }, 0, 0, 0, 0}, // do not remove this one, it clears AND checks for cross-page boundary errors
	{func(b *sys.Bus, addr mirv.Address) error { return b.Write8(addr, 42) }, 42, 42 << 8, 42 << 24, 42 << 56},
	{func(b *sys.Bus, addr mirv.Address) error { return b.Write16(addr, 0xbeef) }, 0xbe, 0xbeef, 0xbeef << 16, 0xbeef << 48},
	{func(b *sys.Bus, addr mirv.Address) error { return b.Write32(addr, 0xdeadbeef) }, 0xde, 0xdead, 0xdeadbeef, 0xdeadbeef << 32},
	{func(b *sys.Bus, addr mirv.Address) error { return b.Write64(addr, 0xbadc0feedeadbeef) }, 0xba, 0xbadc, 0xbadc0fee, 0xbadc0feedeadbeef},
}
...
func BenchmarkBus_Write64(b *testing.B) {
	r := mem.BigEndianRAM(psz)
	bus := sys.NewBus(psz, 1<<8)
	bus.Map(0, r.Pages(psz)...)
	for i := 0; i < b.N; i++ {
...
func BenchmarkMemInterface_Write64(b *testing.B) {
	var r mirv.Memory = mem.BigEndianRAM(psz)
	for i := 0; i < b.N; i++ {
		if err := r.Write64(0, 12345); err != nil {
			b.Fatal(err)
		}
	}
}
